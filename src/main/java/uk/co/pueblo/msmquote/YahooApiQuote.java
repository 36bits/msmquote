...
public class YahooApiQuote implements YahooQuote {
	 ...
	 * @throws IOException
	 */
	private Iterator<JsonNode> getApiQuote(String apiUrl) throws IOException {
		...
		try (InputStream quoteIs = new URL(apiUrl).openStream();) {
			...
			return mapper.readTree(quoteIs).at("/quoteResponse/result").elements();
		}
	}
	...
	public Map<String, Object> getNext() {
		...
		try {		
			...
			if (quoteType.equals(QuoteType.CURRENCY.toString())) {
				...
				symbol = MsmUtils.truncateSymbol(symbol);
				...
				if (quoteType.equals(QuoteType.EQUITY.toString()) || quoteType.equals(QuoteType.BOND.toString()) || quoteType.equals(QuoteType.MUTUALFUND.toString())) {
					...
					if ((quoteDivisorProp = props.getProperty("quoteDivisor." + result.get("currency").asText())) != null) {
				...
				if (quoteType.equals(QuoteType.EQUITY.toString())) {
					...
					if (result.has("trailingAnnualDividendYield")) {
						quoteRow.put("dDividendYield", result.get("trailingAnnualDividendYield").asDouble() * 100 * quoteDivisor);
					} else {
